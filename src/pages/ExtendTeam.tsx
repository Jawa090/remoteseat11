    ...
    <div className="relative">
      <div className="flex h-[350px] gap-2">
        {cards.map((card) => (
          <div
            key={card.id}
            className={`relative rounded-2xl overflow-hidden cursor-pointer transition-all duration-500 ease-in-out ${activeCard === card.id ? 'flex-[3]' : 'flex-1'
              }`}
            style={{
          ...
          >
            ...
            <div className="relative z-10 p-6 text-white flex flex-col justify-end h-full">
              ...
              <div
                className={`font-semibold mb-4 transition-all duration-300 ${activeCard === card.id ? 'text-xl text-center' : 'text-lg'
                  }`}
                dangerouslySetInnerHTML={{ __html: card.title }}
              />

              {/* Expanded Content - Only visible when active */}
              {/* <div className={`transition-all duration-300 ${activeCard === card.id ? 'opacity-100 transform translate-y-0' : 'opacity-0 transform translate-y-4'
                }`}>
                {activeCard === card.id && (
          ...
          <div className="bg-[#dbd4ce] rounded-2xl px-8 py-6 shadow-xl border-2 border-[#f2572b]/20">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                <Link
                  to="/contact-us/"
                  className="inline-flex items-center justify-center gap-2 rounded-full bg-[#f2572b] px-5 py-2.5 text-sm font-medium text-white shadow-sm hover:bg-[#e04a1f] transition-colors"
                >
